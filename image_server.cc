...
public:
    ...
    const std::string& content) { //写文件的函数 
    std::ofstream file(file_name.c_str());
    if (!file.is_open()) {   //文件没打开的情况下
      return false;
    }
    file.write(content.c_str(), content.length());//写入C语言的字符串，写入的长度
    file.close();
    return true;
  }
...
MYSQL* mysql = NULL;//mysql句柄
int main() {
  using namespace httplib;//命令空间，建议放在main函数里
 ...
 // 2. 响应(Response)
  ...
  server.Post("/image", [&image_table](const Request& req, Response& resp) {
      ...
      if (!ret) {
        printf("文件上传出错!\n");
        resp.status = 404; //客户端状态错误码
        // 可以使用 json 格式组织一个返回结果
        resp_json["ok"] = false;
        resp_json["reason"] = "上传文件出错, 没有需要的upload 字段";
        resp.set_content(writer.write(resp_json), "application/json");//Json序列化字符串的结果，返回类型
        return;
      }
      ...
      // 4. 把图片保存到指定的磁盘目录中
			auto body = req.body.substr(file.offset, file.length);//截取文件的起始位置，截取的长度
      FileUtil::Write(image["path"].asString(), body); //写入文件
  ...
  server.set_base_dir("./picture");
  server.listen("0.0.0.0", 9094);//启动服务器，IP地址+端口号
